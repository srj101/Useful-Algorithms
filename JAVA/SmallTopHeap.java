...
public class SmallTopHeap extends Heap {
    public static SmallTopHeap createEmptyHeap(int size) {
        SmallTopHeap r = new SmallTopHeap();
        r.init(size);
        return r;
    }
    ...
    public int removeTop() {
        ...
        while(2 * current + 2 < tail) {
            switch(Simple.minOf(element, data[2 * current + 1], data[2 * current + 2])) {
                case 1:
                    data[current] = element;
                    return r;
                case 2:
